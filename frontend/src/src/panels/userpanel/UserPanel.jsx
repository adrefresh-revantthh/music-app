...
export default function UserPanel() {
  ...
  // Read the "already installed" flag from localStorage on mount, not just from
  // witnessing a live appinstalled event — otherwise a device that installed
  // the app in a *previous* session shows the generic "not available" message
  // instead of "App installed" every time it's opened in a regular browser tab.
  const [installedFlag, setInstalledFlag] = useState(() => {
    try { return localStorage.getItem("loopz_installed") === "1"; } catch { return false; }
  });
  ...
  useEffect(() => {
    ...
    const onInstalled = () => {
      ...
      try { localStorage.setItem("loopz_installed", "1"); } catch {}
      setShowInstallStep(false);
    };
    window.addEventListener("beforeinstallprompt", onBeforeInstall);
    window.addEventListener("appinstalled", onInstalled);

    // If we're actually running standalone right now (launched from the
    // home-screen icon), that alone proves installation happened — persist
    // it even if this device never fired appinstalled while we were listening
    // (e.g. it was installed via the browser's own address-bar icon).
  ...
  // plain function (not depending on component scope) usable inside the mount effect above
  function isStandaloneCheck() {
    try { return window.matchMedia?.("(display-mode: standalone)")?.matches || window.navigator?.standalone === true; }
    catch { return false; }
  }

  // Not every "can't install" case is the same — surface the real reason
  // instead of one flat unhelpful string. The most common real-world case
  // is testing the local dev server from a phone over a plain-http LAN IP
  // (e.g. http://192.168.x.x:5173): Service Workers (and therefore PWA
  // installability) require a secure context — https, or localhost on the
  // same machine — so that will never offer an install prompt no matter
  // what the code does.
  const installUnavailableReason = () => {
    if (typeof window !== "undefined" && window.isSecureContext === false) {
      return "Install needs a secure (https) connection — won't work over plain http, e.g. testing on your phone via a local IP. Try the deployed site instead.";
    }
    return "Install not available in this browser";
  };
        ...
        <div style={{ position:"fixed", inset:0, zIndex:1900, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ width:"100%", maxWidth:360, background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, padding:"32px 28px", animation:"fadeUp 0.3s ease", textAlign:"center" }}>
            ...
            {installEvent ? (
              ...
              <p style={{ fontSize:13, color:C.sub, marginBottom:24, lineHeight:1.6 }}>This page isn't loaded over a secure connection, so one-tap install can't work here (needs https, or localhost on this same computer). Try the deployed site instead.</p>
            ) : (
      ...
      <div style={{ maxWidth:860, margin:"0 auto", padding:"24px 16px" }}>
        ...
        {!loading && tab==="profile" && (
          <div style={{ animation:"slideUp 0.3s ease" }}>
            ...
            {!isStandalone() && (
              <button style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, width:"100%", padding:"13px", borderRadius:12, border:`1px solid ${C.accentBorder}`, background:C.accentDim, color:C.accent, fontWeight:700, fontSize:installedFlag||installEvent?13:12, cursor:installEvent?"pointer":"default", fontFamily:"'Outfit',sans-serif", marginBottom:28, opacity:installEvent||installedFlag?1:0.6, textAlign:"center", lineHeight:1.4 }}
                disabled={!installEvent} onClick={installApp}>
                <FaDownload size={13}/> {installedFlag ? "App installed ✓" : installEvent ? "Download App" : installUnavailableReason()}
              </button>
            )}
